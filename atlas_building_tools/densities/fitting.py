...
def fill_in_homogenous_regions(
    ...
    Args:
        ...
        densities: the data frame returned by
            :fun:`atlas_building_tools.densities.fitting.create_dataframe_from_known_densities`.
...
def linear_fitting_xy(
    ...
    Raises:
       AtlasBuildingToolsError if some of the `sigma`values are negative.
    """
...
def compute_fitting_coefficients(
    ...
    Args:
        ...
        average_intensities: data frame returned by
          :fun:`atlas_building_tools.densities.fitting.compute_average_intensities`.
...
def _check_homogenous_regions_sanity(homogenous_regions: "pd.DataFrame") -> None:
    ...
    if len(diff) > 0:
        raise AtlasBuildingToolsError(f"`homogenous_regions` has unexpected cell types: {diff}")
