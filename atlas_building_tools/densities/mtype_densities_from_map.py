...
from atlas_building_tools.utils import get_layer_masks
...
def check_probability_map_sanity(probability_map: "pd.DataFrame") -> None:
    ...
    Args:
        probability_map:
            ...
            labelled by mtypes.
    Raises:
        AtlasBuildingToolsError if the sum of each row is not (close to) 1.0 or if `probability_map`
        holds a negative value.
    """
    if not np.all(probability_map >= 0.0):
        raise AtlasBuildingToolsError("The probability map has negative values.")
