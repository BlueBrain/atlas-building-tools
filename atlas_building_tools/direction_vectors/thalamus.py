...
def _get_common_outer_boundary(
    mask: NDArray[bool], sub_mask: NDArray[bool]
) -> NDArray[bool]:
    """
...
def compute_direction_vectors(
    ...
    reticular_nucleus_complement_mask = np.logical_and(
        thalamus_mask, ~reticular_nucleus_mask
    )
    common_outer_boundary_mask = _get_common_outer_boundary(
        thalamus_mask, reticular_nucleus_mask
    )
    landscape = {
    ...
    ] = rt_complement_direction_vectors[reticular_nucleus_complement_mask]
    direction_vectors[reticular_nucleus_mask] = rt_direction_vectors[
        reticular_nucleus_mask
    ]
    ...
    if nans > 0.0:
        warnings.warn(
            'NaN direction vectors in {:.5%} of thalamic voxels'.format(float(nans))
        )
