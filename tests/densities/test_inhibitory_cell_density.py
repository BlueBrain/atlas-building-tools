...
from atlas_building_tools.densities.utils import get_group_ids
from atlas_building_tools.exceptions import AtlasBuildingToolsError
import atlas_building_tools.densities.inhibitory_cell_density as tested

TESTS_PATH = Path(__file__).parent.parent


def test_compute_inhibitory_cell_density_internal():
    ...
    actual = tested._compute_inhibitory_cell_density(gad, nrn1, neuron_density, 0.6)
    npt.assert_almost_equal(actual[0], expected[0])
    npt.assert_almost_equal(actual[1], expected[1])

    inhibitory_data = {
        'ratios': {'Cerebellum group': 0.2, 'Isocortex group': 0.2, 'Rest': 0.6,},
        'cell_count': 6,
    }
    ...
    actual = tested._compute_inhibitory_cell_density(
        gad,
        nrn1,
        neuron_density,
...
def test_compute_inhibitory_cell_density_exceptions():
    gad = np.array([[[0.0, 0.0, 0.5, 0.25, 0.25]]])
    nrn1 = np.array([[[0.0, 0.0, 0.4, 0.35, 0.25]]])
    ...
    with pytest.raises(AtlasBuildingToolsError):
        tested._compute_inhibitory_cell_density(
            gad, nrn1, neuron_density, inhibitory_data=inhibitory_data
        )

    with pytest.raises(AtlasBuildingToolsError):
        tested._compute_inhibitory_cell_density(gad, nrn1, neuron_density)
    ...
    with warnings.catch_warnings(record=True) as w:
        tested._compute_inhibitory_cell_density(
            ...
            neuron_density,
            inhibitory_data=inhibitory_data,
            uniform_inhibitory_ratio=0.079,
        )
        msg = str(w[0].message)
        assert 'Using ' in msg and 'only' in msg


def test_compute_inhibitory_cell_density():
    ...
    with patch(
        'atlas_building_tools.densities.inhibitory_cell_density.get_group_ids',
        return_value=group_ids,
    ):
        with patch(
            'atlas_building_tools.densities.inhibitory_cell_density.get_region_masks',
            return_value=region_masks,
        ):
            with patch(
                'atlas_building_tools.densities.inhibitory_cell_density.compensate_cell_overlap',
                side_effect=[gad, nrn1],
            ):
                inhibitory_cell_density = tested.compute_inhibitory_cell_density(
                    region_map,
                    annotation_raw,
                    gad,
                    nrn1,
                ...
                npt.assert_almost_equal(inhibitory_cell_density, expected)
...
def test_compute_inhibitory_density_large_input():
    ...
    inhibitory_data = {
        'ratios': {'Cerebellum group': 0.4, 'Isocortex group': 0.35, 'Rest': 0.25,},
        'cell_count': 30000,
    }
    inhibitory_cell_density = tested.compute_inhibitory_cell_density(
        region_map,
    ...
    assert np.all(inhibitory_cell_density <= neuron_density)
    npt.assert_almost_equal(np.sum(inhibitory_cell_density), 30000)
