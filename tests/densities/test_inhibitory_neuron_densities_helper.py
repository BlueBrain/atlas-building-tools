...
import atlas_building_tools.densities.inhibitory_neuron_densities_helper as tested
...
def test_check_region_counts_consistency():
    ...
    with pytest.raises(AtlasBuildingToolsError, match=r"gad67\+.*exceed"):
        tested.check_region_counts_consistency(region_counts, hierarchy_info)
    ...
    with pytest.raises(
        AtlasBuildingToolsError, match=r"sum of the counts.*pv\+.*exceeds.*region A"
    ):
        tested.check_region_counts_consistency(region_counts, hierarchy_info)
