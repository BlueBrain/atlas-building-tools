...
import atlas_building_tools.densities.inhibitory_neuron_densities_optimization as tested
...
def test_unfeasible_constraint():
    ...
    with pytest.raises(AtlasBuildingToolsError, match="neuron count"):
...
def test_error_on_sigmas_inconsistencies():
    ...
    with pytest.raises(AtlasBuildingToolsError, match="not certain"):
        tested.create_inhibitory_neuron_densities(
            data["hierarchy"],
            data["annotation"],
            data["voxel_volume"],
