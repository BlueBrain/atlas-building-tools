...
def create_placement_hints():
    placement_hints_2 = np.array(
        [[[[3.0, 5.0], [3.0, 5.0], [3.0, 5.0], [3.0, 5.0], [3.0, 5.0]]]])
    placement_hints_3 = np.array(
...
def create_excitatory_neuron_density():
    return VoxelData(
        np.array([[[3.0, 1.0, 4.0, 2.0, 2.0]]]), voxel_dimensions=[1.0] * 3
    )


def create_inhibitory_neuron_density():
    return VoxelData(
        np.array([[[4.0, 3.0, 1.0, 2.0, 1.0]]]), voxel_dimensions=[1.0] * 3
    )
...
def create_input_volumetric_data(dirpath: str):
    ...
    return {
        'excitatory_neuron_density': str(
            Path(dirpath, 'excitatory_neuron_density.nrrd')
        ),
        'inhibitory_neuron_density': str(
            Path(dirpath, 'inhibitory_neuron_density.nrrd')
        ),
...
def create_expected_cell_densities():
    return {
        'L2_TPC:A': np.array(
            [[[0.0, 0.0, 0.0, 2.0 * 10100.0 / 15800.0, 2.0 * 11000.0 / 17200.0]]]
        ),
...
def test_density_profile_collection_loading():
    ...
    expected_profile_data = {
        'layer_2': {
            'inhibitory': pd.DataFrame({'L23_BP': [1.0, 1.0]}).rename(
                index={0: 3, 1: 4}
            ),
...
def test_load_placement_hints():
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        for ph_data, ph_path in zip(
            placement_hints.values(), placement_hints_paths.values()
        ):
            VoxelData(ph_data, voxel_dimensions=[1.0] * 3).save_nrrd(ph_path)

        loaded = tested.DensityProfileCollection.load_placement_hints(
            placement_hints_paths
        )
...
def test_create_mtype_density():
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        created_cell_density = VoxelData.load_nrrd(
            str(Path(tempdir, 'L2_TPC:A_density.nrrd'))
        ).raw
        npt.assert_array_equal(
            created_cell_density, expected_cell_densities['L2_TPC:A']
        )
        density_profile_collection.create_density(
            'L23_BP',
            'inhibitory',
        ...
        created_cell_density = VoxelData.load_nrrd(
            str(Path(tempdir, 'L23_BP_density.nrrd'))
        ).raw
