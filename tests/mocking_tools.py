...
class Mocked_get_region_mask(MockedLoader):
    ...
    def __call__(self, acronym, annotation, hierarchy):
        if acronym.startswith("@"):
            ...
            matches = [acro
                       for acro in self.dict
                       if acronym_regexp.match(acro) is not None]
            return np.any([self(m, annotation, hierarchy)
                           for m in matches], axis=0)
        else:
...
class Mocked_VoxelData_load_nrrd(MockedLoader):
    ...
    def __call__(self, filename):
        return VoxelData(self.dict[filename],
